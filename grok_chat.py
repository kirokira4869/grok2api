...
def resolve_enable_search(
    ...
    return bool(get_config("grok.default_enable_search", False))
...
class ChatRequestBuilder:
    ...
    def build_payload(
        ...
        if enable_search is None:
            enable_search = bool(get_config("grok.default_enable_search", False))
...
class GrokChatService:
    ...
    async def chat_openai(self, token: str, request: ChatRequest):
        ...
        enable_search = resolve_enable_search(
...
class ChatService:
    ...
    async def completions(
        ...
        Args:
            ...
            thinking: 思考模式
            search: 是否启用联网搜索
            enable_search: 是否启用联网搜索（兼容字段）
            tools: 工具列表，支持 web_search / web_search_preview
